...
/**
 * Tests for BigDecimalGreaterEqualOperator.
 */
public class BigDecimalGreaterEqualOperatorTest extends OperatorTest {
  ...
  public void shouldCalculateWithDefaults() {
    ...
    TermTester.testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", TWO).equalTo(ZERO);
    TermTester.testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", ONE).equalTo(ONE);
    TermTester.testThat(createOp(v("a"), v("b"))).with("a", TWO).with("b", ONE).equalTo(ONE);
  }
  ...
  public void shouldCalculateWithNullFocused() {
    ...
    TermTester.testThat(createOp(v("a"), v("b"))).with(nullConfiguration).with("a", ONE).with("b", TWO).equalTo(ZERO);
    TermTester.testThat(createOp(v("a"), v("b"))).with(nullConfiguration).with("a", ONE).with("b", ONE).equalTo(ONE);
    TermTester.testThat(createOp(v("a"), v("b"))).with(nullConfiguration).with("a", TWO).with("b", ONE).equalTo(ONE);
  }
  ...
  public void shouldCalculateWithZeroFocused() {
    ...
    TermTester.testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).equalTo(ONE);
    TermTester.testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", ONE).equalTo(ONE);
    TermTester.testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("b", TWO).equalTo(ZERO);
    TermTester.testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", ONE).with("b", TWO).equalTo(ZERO);
    TermTester.testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", ONE).with("b", ONE).equalTo(ONE);
    TermTester.testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", TWO).with("b", ONE).equalTo(ONE);
  }

  private static Term<BigDecimal> createOp(Term<BigDecimal> base, Term<BigDecimal> applicant) {
    return new BigDecimalGreaterEqualOperator(base, applicant);
  }
