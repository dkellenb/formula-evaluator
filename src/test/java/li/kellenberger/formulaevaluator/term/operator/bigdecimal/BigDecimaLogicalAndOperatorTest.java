...
public class BigDecimaLogicalAndOperatorTest extends OperatorTest {
  ...
  public void shouldCalculateWithDefaults() {
    ...
    testThat(createOp(v("a"), v("b"))).with("a", ZERO).with("b", ONE).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", ZERO).equalTo(ZERO);
  ...
  public void shouldCalculateWithZeroFocused() {
    ...
    testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("b", ONE).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with("a", ZERO).with("b", ZERO).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with("a", ZERO).with("b", ONE).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", ZERO).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", ONE).equalTo(ONE);
