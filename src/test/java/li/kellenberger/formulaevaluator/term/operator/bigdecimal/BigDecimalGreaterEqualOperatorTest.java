...
public class BigDecimalGreaterEqualOperatorTest extends OperatorTest {
  ...
  public void shouldCalculateWithDefaults() {
    ...
    testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", TWO).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with("a", ONE).with("b", ONE).equalTo(ONE);
    testThat(createOp(v("a"), v("b"))).with("a", TWO).with("b", ONE).equalTo(ONE);
  }
  ...
  public void shouldCalculateWithNullFocused() {
    ...
    testThat(createOp(v("a"), v("b"))).with(nullConfiguration).with("a", ONE).with("b", TWO).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with(nullConfiguration).with("a", ONE).with("b", ONE).equalTo(ONE);
    testThat(createOp(v("a"), v("b"))).with(nullConfiguration).with("a", TWO).with("b", ONE).equalTo(ONE);
  }
  ...
  public void shouldCalculateWithZeroFocused() {
    ...
    testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("b", TWO).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", ONE).with("b", TWO).equalTo(ZERO);
    testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", ONE).with("b", ONE).equalTo(ONE);
    testThat(createOp(v("a"), v("b"))).with(zeroConfiguration).with("a", TWO).with("b", ONE).equalTo(ONE);
  }
